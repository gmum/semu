...
def setup_fid_data(class_to_forget, path, image_size, interpolation="bicubic"):
    ...
    # )
    real_set = Imagenette(
        "train", class_to_forget=6, transform=transform
    )
    real_set = [data[0] for data in real_set][:100]
...
def setup_fid_data_all_at_once(class_to_forget, path, image_size, interpolation="bicubic"):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)

    real_set = Fake_Imagenette(f"data/fid_subsets/imagenette_without_label_{class_to_forget}", class_to_forget, transform=transform)
...
def setup_fid_data_all_at_once2(class_to_forget, path, label, image_size, interpolation="bicubic"):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)

    real_set = Fake_Imagenette(f"data/fid_subsets/imagenette_without_label_{label}", label, transform=transform)
    real_set = [data[0] for data in real_set]
...
def create_fid_datasets(class_to_forget, save_path, image_size, interpolation="bicubic"):
    interpolation = INTERPOLATIONS[interpolation]
    transform = get_transform(interpolation, image_size)
    ...
    for cl in range(10):
        real_set = Imagenette(
        "train", class_to_forget=cl, transform=None
    )
        real_set = [data[0] for data in real_set][:250]
        for i in range(len(real_set)):
            im = real_set[i]
            path = f"data/fid_subsets/large/{cl}_{i}.png"
            im.save(path)
            real_set.append(im)
        del real_set
    print("DONE!")
...
if __name__ == "__main__":
    # for i in range(10):
    create_fid_datasets(1, f"imagenette_without_label", 512)
    # create_fid_datasets(1, "imagenette_without_label_6", 512)
