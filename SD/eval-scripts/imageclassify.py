...
if __name__ == "__main__":
    ...
    if save_path is not None:
        ...
        for case in list(set(case_numbers)):
            ...
            print(f"Case: {case}")
            # good_pred = case_subset.query("class.str.lower() == category_top1.str.lower()").count()[0]
            # good_pred = case_subset[case_subset['class'].to_string().lower() == case_subset['category_top1'].to_string().lower()].count()[0]
            good_pred = 0
            for k in range(all_case_rows):
                if case_subset["class"].iloc[k].lower() == case_subset["category_top1"].iloc[k].lower():
                    good_pred += 1
            print(f"UA: {1 - (good_pred / all_case_rows)}")
