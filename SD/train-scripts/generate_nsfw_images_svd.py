...
EXAMPLES = [296, 327, 541, 622, 698, 1066, 1126, 1276, 1094, 1477]
# SUBSET = [296, 541, 1066, 1126, 1276, 1094]
SUBSET = [296, 327, 541, 622, 698, 1066, 1126, 1276, 1094, 1477]

def generate_nsfw_images(
    diffuser,
    model_name,
    prompts_path,
    save_path,
    ...
    for _, row in df.iterrows():
        ...
        if case_number not in EXAMPLES:
            continue
        ...
        for i in range(10):
            text_input = diffuser.tokenizer(
                prompt,
                padding="max_length",
                max_length=diffuser.tokenizer.model_max_length,
                truncation=True,
                return_tensors="pt",
            )

            text_embeddings = diffuser.text_encoder(text_input.input_ids.to(device))[0]

            max_length = text_input.input_ids.shape[-1]
            uncond_input = diffuser.tokenizer(
                [""] * batch_size,
                padding="max_length",
                max_length=max_length,
                return_tensors="pt",
            )
            uncond_embeddings = diffuser.text_encoder(uncond_input.input_ids.to(device))[0]

            text_embeddings = torch.cat([uncond_embeddings, text_embeddings])

            latents = torch.randn(
                (batch_size, diffuser.unet.in_channels, height // 8, width // 8),
                generator=generator,
            )
            latents = latents.to(device)

            diffuser.scheduler.set_timesteps(num_inference_steps)

            latents = latents * diffuser.scheduler.init_noise_sigma

            from tqdm.auto import tqdm

            diffuser.scheduler.set_timesteps(num_inference_steps)

            for t in tqdm(diffuser.scheduler.timesteps):
                # expand the latents if we are doing classifier-free guidance to avoid doing two forward passes.
                latent_model_input = torch.cat([latents] * 2)

                latent_model_input = diffuser.scheduler.scale_model_input(
                    latent_model_input, timestep=t
                )

                # predict the noise residual
                with torch.no_grad():
                    noise_pred = diffuser.unet(
            ...
            with torch.no_grad():
                image = diffuser.vae.decode(latents).sample
