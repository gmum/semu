...
class CustomLinear(nn.Linear):
    def __init__(self, m: nn.Linear, u: Tensor, vh: Tensor, n_imp_comps: Tensor = None):
        super().__init__(
        ...
        self.register_buffer("n_imp_comps", n_imp_comps)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        _weight = self.a.clone() + torch.einsum("ij,jk,kl->il", self.u, weight, self.vh)
        return nn.functional.linear(my_x, _weight, self.bias).clone()


class CustomConv2d(nn.Conv2d):
    def __init__(self, m: nn.Conv2d, u: Tensor, vh: Tensor, n_imp_comps: Tensor = None):
        super().__init__(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        my_x = x.clone()
        weight = self.weight.clone()
        _weight = torch.einsum("nij,njk,nkl->nil", self.u, weight, self.vh)
        _weight = torch.unflatten(_weight, 2, self.kernel_size)
        _weight = torch.permute(_weight, (1, 0, 2, 3))
        _weight = self.a.clone() + _weight
        return self._conv_forward(my_x, _weight, self.bias).clone()
...
def replace_layers_with_custom(
    ...
    u: Dict[str, Tensor],
    vh: Dict[str, Tensor],
    num_imp_comps: Dict[str, Tensor],
    parent_name: str = "",
    attention_only: bool = False,
    others_only: bool = False,
    changed_layers_class: List[str] = None,
    requires_grad_list: List[str] = None,
) -> nn.Module:
    """
    ...
    Args:
        ...
        num_imp_comps (dict[str, Tensor]): Dictionary of the number of important components in the weight tensor
                                           that have been gradient-changed during unlearning
    ...
    attention_property = lambda x: "attn" in x if attention_only else True
    others_property = lambda x: "attn" not in x if others_only else True

    for name, module in model.named_children():
        full_name = f"{parent_name}.{name}" if parent_name else name
        if type(module) is nn.Conv2d:
            full_name_weight = f"{full_name}.weight"
            if full_name_weight in requires_grad_list:
                if "conv2d" in  changed_layers_class and not any(forbidden in full_name for forbidden in forbidden_list):
                    # Replace Conv2d with CustomConv2d
                    setattr(
                        model,
                        name,
                        CustomConv2d(
                            module, u[full_name_weight], vh[full_name_weight], num_imp_comps[full_name_weight]
                        ),
                    )
        elif type(module) is nn.Linear:
            full_name_weight = f"{full_name}.weight"
            if full_name_weight in requires_grad_list:
                if "linear" in changed_layers_class and not any(forbidden in full_name for forbidden in forbidden_list):
            ...
            replace_layers_with_custom(module, u, vh, num_imp_comps, full_name, attention_only, changed_layers_class, requires_grad_list=requires_grad_list)


def get_requires_grad_list(
    module,
    changed_layers_class: List[str] = None,
    ...
    requires_grad_list = []
    not_requires_grad_list = []

    forbidden_list = ["norm"]

    # Recursive function to set requires_grad
    def _get_requires_grad_list(module, our_list, our_not_list, prefix=""):
        for name, child in module.named_children():
            ...
            for name_param, param in child.named_parameters():
                if not include_bias and name_param.lower().endswith("bias"):
                    our_not_list.append(f"{child_name}.{name_param}")
                else:
                    our_list.append(f"{child_name}.{name_param}")

            # Recurse into child modules
            _get_requires_grad_list(child, our_list, our_not_list, child_name)

    # Start the recursive setting of requires_grad
    _get_requires_grad_list(module, requires_grad_list, not_requires_grad_list)

    requires_grad_list = [x for x in requires_grad_list if x not in not_requires_grad_list]
    requires_grad_list = [x[:-7] if x.endswith("weight") else x for x in requires_grad_list]
    requires_grad_list = [x for x in requires_grad_list if not any(forbidden in x for forbidden in forbidden_list)]
    return list(set(requires_grad_list))
