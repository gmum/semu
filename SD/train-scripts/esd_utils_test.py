...
class StableDiffuser(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.vae = AutoencoderKL.from_pretrained(
            "CompVis/stable-diffusion-v1-4", subfolder="vae", cache_dir="huggingface_test", force_download=True)
        
        # Load the tokenizer and text encoder to tokenize and encode the text.
        self.tokenizer = CLIPTokenizer.from_pretrained(
            "openai/clip-vit-large-patch14", cache_dir="huggingface_test", force_download=True)
        self.text_encoder = CLIPTextModel.from_pretrained(
            "openai/clip-vit-large-patch14", cache_dir="huggingface_test", force_download=True)
        
        # The UNet model for generating the latents.
        self.unet = UNet2DConditionModel.from_pretrained(
            "CompVis/stable-diffusion-v1-4", subfolder="unet", cache_dir="huggingface_test", force_download=True)
        
        self.feature_extractor = CLIPFeatureExtractor.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="feature_extractor", cache_dir="huggingface_test", force_download=True)
        # self.safety_checker = StableDiffusionSafetyChecker.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="safety_checker", cache_dir="huggingface_diffusers")

        if scheduler == 'LMS':
            self.scheduler = LMSDiscreteScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear", num_train_timesteps=1000)
        elif scheduler == 'DDIM':
            self.scheduler = DDIMScheduler.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="scheduler", cache_dir="huggingface_test", force_download=True)
        elif scheduler == 'DDPM':
            self.scheduler = DDPMScheduler.from_pretrained("CompVis/stable-diffusion-v1-4", subfolder="scheduler", cache_dir="huggingface_test", force_download=True)    
        ...
        self.unet.config.use_cache = False
    ...
    def add_batch_noises(self, latents, noise, steps):

        return self.scheduler.add_noise(latents, noise, steps)
    ...
    def get_text_embeddings(self, prompts, neg_prompts=None, n_imgs=1):

        text_tokens = self.text_tokenize(prompts)

        text_embeddings = self.text_encode(text_tokens).unsqueeze(1)

        if neg_prompts is not None:
            
            neg_text_tokens = self.text_tokenize(neg_prompts)

            neg_text_embeddings = self.text_encode(neg_text_tokens).unsqueeze(1)

            text_embeddings = torch.cat([neg_text_embeddings, text_embeddings], dim=1)

        else:
            unconditional_tokens = self.text_tokenize([""] * len(prompts))

            unconditional_embeddings = self.text_encode(unconditional_tokens).unsqueeze(1)

            text_embeddings = torch.cat([unconditional_embeddings, text_embeddings], dim=1)

        if n_imgs > 1:
             text_embeddings = text_embeddings.repeat_interleave(n_imgs, dim=0)

        return text_embeddings
    

    def get_single_text_embeddings(self, prompts, n_imgs=1):
    ...
    def predict_noise_mine(self,
        ...
        for it, time in enumerate(iterations):
            latent = torch.cat([latents[it].unsqueeze(0)] * 2)
            new_latent = self.scheduler.scale_model_input(
                latent, time)
            # predict the noise residual
            preds = self.unet(
                new_latent, time, encoder_hidden_states=text_embeddings[it]).sample

            noise_prediction.append(preds)
            null_out, forget_out = preds.chunk(2)
            null_outs.append(null_out)
            forget_outs.append(forget_out)

        null_outs = torch.cat(null_outs)
        forget_outs = torch.cat(forget_outs)

        noise_prediction = (1 + guidance_scale) * forget_outs - guidance_scale * null_outs

        return noise_prediction
    

    def predict_outs(self,
             iterations,
             latents,
             text_embeddings,
             ):
        ...
        for it, time in enumerate(iterations):
            ...
            pseudo_out, forget_out = preds.chunk(2)
            pseudo_outs.append(pseudo_out.detach())
            forget_outs.append(forget_out)

        pseudo_outs = torch.cat(pseudo_outs)
        forget_outs = torch.cat(forget_outs)

        return forget_outs, pseudo_outs


    def predict_out_noise(self,
             iterations,
             latents,
             text_embeddings,
             ):
        ...
        for it, time in enumerate(iterations):
            ...
            out_noises.append(preds)

        out_noises = torch.cat(out_noises)
    ...
    #     # predict the noise residual
    #     noise_prediction = self.unet(
    #         latents, steps, encoder_hidden_states=text_embeddings).sample
