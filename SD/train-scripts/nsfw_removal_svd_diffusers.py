...
from generate_svd_diffusers import generate_svd, generate_nsfw_svd
from svd_utils_diffusers import clip_grad, transform_text_layer, CustomLinear, CustomConv2d, set_requires_grad, train_phase
from generate_images_svd import generate_images
from generate_nsfw_images_svd import generate_nsfw_images, generate_nsfw_images_grid
...
def nsfw_removal(
    ...
    diffuser, trainable_params = generate_nsfw_svd(c_guidance=args.c_guidance, batch_size=batch_size, 
    ...
    diffuser.to(device)
    prompts_path = "prompts/unsafe-prompts4703.csv"
    model_name = f"nsfw_method_{train_method}-alpha_{alpha}-epoch_{epochs}-lr_{lr}-explained_variance_ratio_{args.explained_variance_ratio}-use_projection_grad_{args.use_projection_grad}-retrain_{args.retrain}-retrain_method_{args.retrain_method}"
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % args.sample_every == 0:
            ...
            if args.add_name is not None:
                generate_nsfw_images(diffuser=diffuser, model_name=model_name, prompts_path=prompts_path, save_path=f"{args.add_name}/intermediate_images/{model_name}/epoch_{epoch+1}", device=device, num_samples=1)
            else:
                generate_nsfw_images_grid(diffuser=diffuser, model_name=model_name, prompts_path=prompts_path, save_path=f"NEW_results_nsfw_grid/intermediate_images/{model_name}/epoch_{epoch+1}", device=device, num_samples=1)


    diffuser.eval()
    if args.add_name is not None:
        generate_nsfw_images(diffuser=diffuser, model_name=model_name, prompts_path=prompts_path, save_path=f"{args.add_name}/final_images/{model_name}", device=device, num_samples=10)
    else:
        generate_nsfw_images(diffuser=diffuser, model_name=model_name, prompts_path=prompts_path, save_path=f"NEW_results_nsfw_grid/final_images/{model_name}", device=device, num_samples=10)
    save_model(
        diffuser,
        model_name,
        None
...
def save_model(
    ...
    # SAVE MODEL
    folder_path = f"nsfw_models_new/{name}"
