...
def parse_args():
    ...
    # -----------------------    our parameters    -----------------------
    parser.add_argument("--explained_variance_ratio", default=None, type=float, help="explained_variance_ratio for SVD")
    parser.add_argument("--use_retaining_data", action="store_true", help="whether using retaining data")
    parser.add_argument("--early_exit", action="store_true", help="whether using early exit")
    parser.add_argument("--early_exit_patience", default=5, type=int, help="early exit patience")
    parser.add_argument("--early_exit_min_delta", default=0.01, type=float, help="early exit min delta")
    parser.add_argument("--use_projection_grad", action="store_true", help="whether using projection grad")
    parser.add_argument("--weight_data_forget", type=float, default=None, help="weight of data forget in loss function")
    parser.add_argument("--pretrained", action="store_true", help="whether using pretrained model")

    return parser.parse_args()


# Save the Namespace object to a file
def save_namespace(namespace, filename):
    """
    Save an argparse.Namespace object to a file using pickle.

    :param namespace: The argparse.Namespace object to save.
...
# Load the Namespace object from a file
def load_namespace(filename):
    """
    Load an argparse.Namespace object from a file using pickle.

    :param filename: The name of the file to load the object from.
    :return: The loaded argparse.Namespace object.
    """
