...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--subset_size", type=int, default=None, help="the size of the subset that will be used for unlearning"
    )
    parser.add_argument(
        "--classifier_evaluation", default=False, action='store_true', help="whether to evaluate the method with the classifier"
    )
    parser.add_argument(
        "--evaluation_samples", type=int, default=500, help="how many samples use for evaluation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval_freq", type=int, default=1000, help="classifier evaluation frequency"
    )
...
def main():
    ...
    try:
        ...
        elif args.mode == "generate_svd":
            runner = Diffusion(args, config)
            runner.generate_svd()
        elif args.mode == "semu_unlearn":
            runner = Diffusion(args, config)
            runner.semu_unlearn()
        elif args.mode == "semu_unlearn_retrain":
            runner = Diffusion(args, config)
            runner.semu_unlearn_retrain()
    except Exception:
