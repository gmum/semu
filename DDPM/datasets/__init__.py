...
def get_forget_dataset(args, config, label_to_drop, subset_size=None):
    """
    Returns vanilla CIFAR10/STL10 dataset.
    """
    if config.data.random_flip is False:
    ...
    if subset_size is not None:
        if subset_size > len(data_remain) or subset_size > len(data_forget):
            print("Subset size is larger than the number of samples in the dataset.")
            subset_size = min(len(data_remain), len(data_forget))
        remain_indices = torch.randperm(len(data_remain))[:subset_size]
        forget_indices = torch.randperm(len(data_forget))[:subset_size]
        data_remain = Subset(data_remain, remain_indices)
        data_forget = Subset(data_forget, forget_indices)
