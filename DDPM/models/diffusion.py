...
class Conditional_Model(nn.Module):
    def __init__(self, config):
        ...
        self.in_channels = in_channels
        self.cond_drop_prob = config.model.cond_drop_prob
...
class Unlearned_Conditional_Model(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.diffusion_model = Conditional_Model(config)
        # self.diffusion_model.load_state_dict(torch.load(config.model.ckpt_path))
        self.diffusion_model.eval()
        self.diffusion_model.requires_grad_(False)

    def forward(self, x, t, c, mode, cond_scale=1.0):
        return self.diffusion_model(x, t, c, mode=mode, cond_scale=cond_scale)
