...
def replace_layers_with_custom(
    ...
    for name, module in model.named_children():
        ...
        if type(module) is nn.Conv2d:
            if "conv2d" in  changed_layers_class and attention_property(full_name) and others_property(full_name):
                # Replace Conv2d with CustomConv2d
                setattr(
                    model,
                    name,
                    CustomConv2d(
                        module, u[full_name], vh[full_name], num_imp_comps[full_name]
                    ),
                )
        elif type(module) is nn.Linear:
            if "linear" in changed_layers_class and attention_property(full_name) and others_property(full_name):
            ...
            replace_layers_with_custom(module, u, vh, num_imp_comps, full_name, attention_only, others_only, changed_layers_class)
